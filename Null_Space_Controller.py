...
class image_converter:
  ...
  def __init__(self):
    ...
    self.box = message_filters.Subscriber("/target2/joint_states",JointState)
    ##### initiate topics ####
  ...
  def calc_cost(self,q1,q2,q3,q4,pos_box):
    F_K = self.F_K_full(q1, q2, q3, q4)
    pos = F_K[:3, -1]
    cost = np.linalg.norm(pos - pos_box)**2
    return cost

  def calc_cost_derivative(self, q, pos_box):
    cost = self.calc_cost(q[0], q[1], q[2], q[3],pos_box)
    dev_array = []
    joint = q[0] + 0.05
    d_cost1 = (self.calc_cost(joint, q[1], q[2], q[3],pos_box))
    dev_array.append((d_cost1-cost)/0.05)
    joint = q[1] + 0.05
    d_cost2 = (self.calc_cost(q[0], joint, q[2], q[3],pos_box))
    dev_array.append((d_cost2-cost)/0.05)
    joint = q[2] + 0.05
    d_cost3 = (self.calc_cost(q[0], q[1], joint, q[3],pos_box))
    dev_array.append((d_cost3-cost)/0.05)
    joint = q[3] + 0.05
    d_cost4 = (self.calc_cost(q[0], q[1], q[2], joint,pos_box))
    dev_array.append((d_cost4-cost)/0.05)      
    return np.array(dev_array)
     
  def null_controller(self,pos_d, q, pos_box):#,image):        
    # P gain
    ...
    pos = F_K[:3, -1]
    cost = self.calc_cost(q[0], q[1], q[2], q[3],pos_box)
    dev_cost = self.calc_cost_derivative(q, pos_box)
    print('derivative array: ', dev_cost)
    print('box position', pos_box)
    print('Cost Function:', cost)
    print('Position of end effector',pos)  
    print ('Position of Target', pos_d)
    dev_cost_trans = np.transpose(dev_cost)
    k_cost = 10
    dq0 = k_cost * dev_cost_trans
    print('dq0: ', (dq0))
    ...
    jacob = self.get_jacobian(q[0], q[1], q[2], q[3])
    print('Jacobian: ', jacob)
    J_inv = np.linalg.pinv(jacob)  # calculating the psudeo inverse of Jacobian
    print ("Pseudoinverse: ", J_inv)
    ####Null space matrix operations ####
    mat_q0 = np.identity(4) - np.matmul(J_inv, jacob)
    print ("mat_q0: ", mat_q0)
    
    dq_d =np.dot(J_inv, ( np.dot(K_d,self.error_d.transpose()) + np.dot(K_p,self.error.transpose()) ) )  # control input (angular velocity of joints)
    print ('dq_d: ', dq_d)
    dq_d = dq_d + np.dot(mat_q0,dq0)
    print ('dq_d: ', dq_d)
  ...
  def callback(self,image1, image2, angles, box):
...
### Control stuff###
      ...
      pos_box = np.array([box.position[0], box.position[1], box.position[2]])    
      q_d, end_position = self.null_controller(pos_sphere_meter_global, q, pos_box)
      print ('q_d ', q_d)
...
def main(args):
  ...
  try:
    ts = message_filters.ApproximateTimeSynchronizer([img_cvt.image_sub1, img_cvt.image_sub2, img_cvt.angles, img_cvt.box], 10, 0.1)
