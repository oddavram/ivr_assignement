...
class image_converter:
  ...
  def __init__(self):
    ...
    self.angles = message_filters.Subscriber("/ja_pub",Float64MultiArray)
...
### Control stuff###
      q = np.array([0, angles.data[0], angles.data[1], angles.data[2]])     
...
###############################################################
      ...
      print(e)
      print ('Angles: ', angles.data[0], angles.data[1], angles.data[2])
      #move robot a bit 
      self.joint1=Float64()
      self.joint1.data= 0.1
      self.joint2=Float64()
      self.joint2.data= angles.data[0] +0.1
      self.joint3=Float64()
      self.joint3.data= angles.data[1] +0.1
      self.joint4=Float64()
      self.joint4.data= angles.data[2] +0.1 
      self.robot_joint1_pub.publish(self.joint1)
      self.robot_joint2_pub.publish(self.joint2)
      self.robot_joint3_pub.publish(self.joint3)
      self.robot_joint4_pub.publish(self.joint4)
...
def main(args):
  ...
  try:
    ts = message_filters.ApproximateTimeSynchronizer([img_cvt.image_sub1, img_cvt.image_sub2, img_cvt.angles], 10, 0.1, allow_headerless = True)
